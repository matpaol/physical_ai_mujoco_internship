...
class EnvRunner:
    ...
    def advance(self) -> None:
        ...
        action = casuale(self.env, self.info, self.generator)
        removed = self.env.unwrapped.object_ids[action]
        ...
        if terminated or truncated:
            self.outcome = "SUCCESSO" if self.info.get("is_success") else "FALLIMENTO"
...
def overlay_text(runner: EnvRunner) -> np.ndarray:
    ...
    for line_number, (line, colour) in enumerate(lines):
        ...
        cv2.putText(canvas, line, position, cv2.FONT_HERSHEY_SIMPLEX, 0.38, colour, 1)
    return canvas
