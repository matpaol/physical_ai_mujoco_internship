...
def prova_ribaltamento() -> None:
    ...
    misurato = _angolo_critico(
        lambda angolo: (
            _scivola_o_si_ribalta(angolo, semi, (2.0, 0.005, 0.005))[1]
            > math.radians(20.0)
        )
    )
    _riga(
...
def prova_inerzia() -> None:
    ...
    casi = (
        (
            ...
            'type="box" size="0.04 0.025 0.015"',
        ),
        (
            "cylinder",
            {"radius": 0.03, "height": 0.09},
            'type="cylinder" size="0.03 0.045"',
        ),
...
def prove_sulle_pile(scene: int) -> None:
    ...
    for seed in range(scene):
        env = gym.make(
            "TargetExtraction-v0", disable_env_checker=True, obs_mode="state"
        )
        ...
        for indice in range(simulatore.data.ncon):
            penetrazione = max(
                penetrazione, -float(simulatore.data.contact[indice].dist)
            )
...
def _riga(
    nome: str, misurato: float, atteso: float, unita: str, tolleranza: float
) -> None:
