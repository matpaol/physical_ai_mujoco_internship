...
def scegli_azione(politica: str, interno, info, rng) -> int:
    ...
    if politica in {"random", "top", "target"}:
        ...
        return {"random": casuale, "top": alto, "target": target}[politica](
            interno, info, rng
        )
...
def stampa_riepilogo(risultati: list[dict], politica: str, object_count: int) -> None:
    ...
    print("=" * LARGHEZZA)
    print(
        f"RIEPILOGO   politica '{politica}'   {object_count} oggetti   {totale} episodi"
    )
    print("-" * LARGHEZZA)
    print(
        f"  successi          {successi}/{totale}  ({100.0 * successi / totale:.0f}%)"
    )
    print(
...
def ispeziona(
    ...
    try:
        for episodio in range(episodes):
            ...
            risultati.append(esegui_episodio(env, policy, seme, rng, verboso=not quiet))
