...
Alla fine confronta il modello con le politiche di riferimento, che e' l'unico
modo di sapere se ha imparato qualcosa: su questo compito `alto` prende +0,808
e l'ottimo +0,886, quindi un modello sotto +0,808 non sta battendo una riga di
euristica.
"""
...
def crea_env(oggetti: int, seed: int | None = None, magazzino: int = 0,
             fresche: float = 0.1):
...
from scripts.politiche import alto, casuale  # noqa: E402


def valuta(scegli, oggetti: int, scene: int) -> np.ndarray:
    """Confronto su scene COSTRUITE DA ZERO, mai passate dal magazzino.

    E' la verifica che la policy abbia imparato il compito e non le scene su
    cui e' stata allenata.
    """
    env = gym.make(
        "TargetExtraction-v0",
        disable_env_checker=True,
    ...
    try:
        for seed in range(scene):
            osservazione, info = env.reset(seed=seed)
            rng = np.random.default_rng(seed)
            ritorno, finito = 0.0, False
            while not finito:
                azione = scegli(env, info, rng, osservazione)
                osservazione, ricompensa, terminato, troncato, info = env.step(
                    azione
                )
                ritorno += ricompensa
                finito = terminato or troncato
            ritorni.append(ritorno)
    finally:
        env.close()
    return np.array(ritorni)
...
def main() -> None:
    ...
    print(f"Confronto su {argomenti.scene_di_prova} scene mai viste in allenamento:\n")
