...
MENU = [
    (
        ...
        "Il robot sceglie e rimuove gli oggetti, un episodio dopo l'altro.\n"
    ...
    (
        ...
        "     modello con le politiche di riferimento su scene mai viste:\n"
...
def chiedi_politica():
    ...
    scelta = ask_int("Chi decide", default=2 if modelli else 1, low=1, high=3 if modelli else 2)
    if scelta == 1:
...
def main() -> int:
    print_banner()
    actions = {key: action for key, _, _, action in MENU if action}
...
def watch_gymnasium(scegli=None) -> None:
    ...
    quando si allena.
    """
    from scripts.run_phase_0b import run_episode
...
def watch_agent() -> None:
    ...
    object_count = ask_int("Quanti oggetti nella scena?", default=3, low=1, high=12)
    parallel = ask_int(
        "Quanti episodi mostrare affiancati?", default=1, low=1, high=9
    )
    episodes = ask_int(
...
def inspect_episode() -> None:
    ...
    """
    from scripts.inspect_episode import DESCRIZIONE_POLITICHE, POLITICHE, ispeziona
...
def print_banner() -> None:
    ...
    try:
        ...
        print(f"  MuJoCo {mujoco.__version__} | Gymnasium {gymnasium.__version__}")
    except ImportError:
